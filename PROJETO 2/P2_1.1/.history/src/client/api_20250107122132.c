...
int kvs_subscribe(const char *key) {
  ...
  if(response_msg[1] == 0){
    printf("Server returned 0 for operation: subscribe (key does not exist)\n");
  } else if (response_msg[1] == 1){
    printf("Server returned 1 for operation: subscribe (key exists)\n");
  } else {
...
int kvs_unsubscribe(const char *key) {
  ...
  char *msg = malloc(sizeof(char) * (1 + 40));
  msg[0] = '4';
  strncpy(msg + 1, key, strlen(key));
  for(i = strlen(key) + 1; i < 1 + 40; i++)
  {
    msg[i] = '\0';
  }
  if(write(request, msg, sizeof(char) * (1 + 40) == -1))
  {
  ...
  if (read(response, &result, sizeof(result)) <= 0 || result != 0) {
    fprintf(stderr, "[ERR]: unsubscription failed for key: %s\n", key);
    return 1;
  }
