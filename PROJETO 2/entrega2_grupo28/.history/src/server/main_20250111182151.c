...
static void dispatch_threads(DIR *dir) {
  ...
  while(1){
    ...
    result = read_all(fifo_de_registo_fd, request + 1 , sizeof(request) - 1, NULL);
    if(result == 0){continue;}
    else if (result < 0) {
      fprintf(stderr, "Failed to read complete registration request\n");
      break;
    }

    ret = read(fifo_de_registo_fd, request, sizeof(request));
    switch (ret){

        case -1:
            if(errno == EINTR) {continue;}
            fprintf(stderr, "Invalid request message format.\n");
            exit(EXIT_FAILURE);
            break;
        ...
        case 0:
            ...
            while(1){
              fifo_de_registo_fd = open(registration_fifo, O_RDONLY | O_WRONLY);
        ...
        case 121:
            if(request[0] < '1' || request[0] > '4') {
              fprintf(stderr, "Wrong OP_CODE for this pipe\n");
              continue;
            }
            
            char req_pipe[41], resp_pipe[41], notif_pipe[41];
            strncpy(req_pipe, request + 1, 40);
            strncpy(resp_pipe, request + 41, 40);
            strncpy(notif_pipe, request + 81, 40);
            req_pipe[40] = '\0';
            resp_pipe[40] = '\0';
            notif_pipe[40] = '\0';
            ...
            switch (request[0]) {
              ...
              case '2' : //Disconnect
                  ...
                  //remover todas as subscriçoes do cliente

                  response[1] = '0'; 
                  break;
              
              case '3': //Subscribe
                  if(key_exists(kvs_table, request[1]) == 1){
                    response[1] = '1';
                    subscriptions[var] = request[1];
                    var++;
                  } else { response[1] = '0'; }
                  break;

              case '4': //Unsubscribe 
                  // cliente indica ao servidor que não quer mais receber notificações de uma chave
                  int flag = 0;
                  for(int k = 0; k < var; k++){
                    if(strcmp(subscriptions[k], request[1]) == 0){
            ...
            break;
        default:
            fprintf(stderr, "Unexpected message size: %d bytes\n", ret);
            break; 
    }
  }
...
int main(int argc, char **argv) {
  ...
  request = malloc(sizeof(char) * (1+40+40+40));
  if(request == NULL) {
