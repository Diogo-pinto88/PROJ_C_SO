...
int kvs_connect(char const* req_pipe_path, char const* resp_pipe_path, char const* server_pipe_path,
  ...
  char *msg = malloc(sizeof(char) * (1+40+40+40));
  msg[0] = '1';
  strncpy(msg + 1, req_pipe_path, strlen(req_pipe_path));
  for(i = strlen(req_pipe_path) + 1; i < 1 + 40; i++)
  {
    msg[i] = '\0';
  }

  strncpy(msg + 1 + 40, resp_pipe_path, strlen(resp_pipe_path));
  for(i = strlen(resp_pipe_path) + 1 + 40; i < 1 + 40 + 40; i++)
  {
    msg[i] = '\0';
  }

  strncpy(msg +1 + 40 + 40, notif_pipe_path, strlen(notif_pipe_path));
